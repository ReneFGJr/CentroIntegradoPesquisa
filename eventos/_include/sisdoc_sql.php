...
function db_erro() {
	global $base, $rlt;
	if ($base == 'pgsql') {
		return (pg_error() . '<BR>' . $rlt);
	}
	if ($base == 'mssql') {
		return (mssql_get_last_message() . '<BR>' . $rlt);
	}
}
...
function db_query($rlt) {
	...
	////////////////////////////// PostGre
	if ($base == 'pgsql') {
		if (strlen($debug) > 0) { $xxx = pg_query($rlt) or die($rlt . pg_email_erro($rlt));
		} else { $xxx = pg_query($rlt) or die('Erro de base <BR>' . pg_email_erro($rlt));
		}
	}
	////////////////////////////// MySQL
	if ($base == 'mysqlPDO') {
	...
	if ($base == 'mssql') {
		$rlt = sql_convert($rlt);
		$sql_query = $rlt;
		if (strlen($debug) > 0) { $xxx = mssql_query($rlt) or die(pg_error() . '<BR>' . $rlt);
		} else { $xxx = mssql_query($rlt);
		}
	}

	if ($base == 'sybase') { $xxx = sybase_query($rlt) or die(pg_error() . '<BR>' . $rlt);
	}
	return $xxx;
}
...
function db_read($rlt) {
	global $base, $conn;
	if ($base == 'pgsql') { $xxx = pg_fetch_array($rlt);
	}
	if ($base == 'mysql') { $xxx = mysql_fetch_array($rlt);
	}
	if ($base == 'mssql') { $xxx = mssql_fetch_array($rlt);
	}
	if ($base == 'mysqlPDO') {
