...
class Central_declaracao_modelos extends CI_model {
	...
	function cp() {
		...
		array_push($cp, array('$T50:7', 'cdm_body_padrao', msg('lb_cdm_body_padrгo'), false, True));
		array_push($cp, array('$O 1:SIM&0:NГO', 'cdm_ativar_boryPadrao', msg('lb_cdm_ativar_padrao'), True, True));
	...
	function modelo_declaracao_view($id = 0, $data) {
		...
		$body = troca($body, '$cargo', 'Coordenadora da Iniciaзгo Cientнfica');
