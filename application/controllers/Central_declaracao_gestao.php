...
class Central_declaracao_gestao extends CI_Controller {
	...
	function cab() {
		...
		/* Menu */
		$menus = array();
		array_push($menus,array('Inэcio','index.php/Central_declaracao_gestao'));
	...
	function index($id = 0) {
		...
		/* Libera Menus */
		if (perfil('#ADM') == 1) {
			array_push($menu, array('Declaraчѕes', 'Manutenчуo de Declaraчѕes', 'ITE', '/central_declaracao_gestao/criar_modelo'));
			array_push($menu, array('Declaraчѕes', 'Imprimir Declaraчуo ou certificado', 'ITE', '/central_declaracao'));
		}
